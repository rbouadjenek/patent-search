...
\label{cha:analysis}
In this chapter, we investigate the problem from term analysis perspective for both patent query and relevant documents because we are interested 
...
Finally, we show that we can get improved using an interactive approach, which needs minimum user~effort~\citep{mona2015termselection}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%% SECTION 1 %%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
main cause for low effective patent prior art search in previous studies~\citep{roda2010clef, magdy2012toward}. 
...
appear in all relevant documents, but explicitly discounts all terms that occur in irrelevant documents~\citep[p.182]{manning2008introduction}.
...
\protect\caption{Scatter plots of recall versus the existence of useful terms in query for different values of $\tau$.}
\label{fig:overlap-r}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\protect\caption{Scatter plots of average precision (AP) versus the existence of useful terms in query for different values of $\tau$.}
\label{fig:overlap-p}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Figure \ref{fig:oracular-a} shows that the performance changes by the values of $\tau$ and peaks at $\tau=0$ for the MAP. We also remark that the performance jumps notably over the baseline for the oracular query formulated by the Equation \ref{eq:oq}. Second, we seek for the best number of terms to formulate the oracular query. 
...
\ \\
We compare oracular queries versus the baseline as shown in Figure \ref{fig:oracularpq}.  Notably,
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In standard IR, removing terms appearing highly frequently across documents in the collection can improve retrieval effectiveness~\citep[p.27]{manning2008introduction}. Inspired by this fact, we hypothesise that we will improve the performance by pruning out highly frequent terms in top 100 retrieved documents after an initial run of the patent query. To identify highly frequent terms, we calculate the average term frequency over top 100 documents for each term --- document frequent ($\mathit{DF}$) score --- as follows:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
 DF(t, Q)=\frac{1}{100}\sum_{d\in  D} c(t, d),    
 \label{eq:df}
\end{equation}
where $D=\{d\in \mbox{Top 100 retrieved documents}\}$, and $c(t, d)$ is the term frequency of each term in document $d$.
%\begin{displaymath}   t\in \lbrace \mbox{terms in top-100 retrieved documents}\rbrace\end{displaymath}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Overall, removing document frequent terms from patent query does not consider an appropriate approach since it hurts the performance. 
...
%\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Automated Techniques Fail to Approximate Oracular Patent Query}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure}[t!]
%\begin{centering}
...
First, we use an anecdotal example --- a sample query --- to analyse terms selected by proposed query reduction methods. 
...
Hence in $\mathit{DF}$ pruning method, if we prune a document frequent term out, we are not essentially removing an unimportant term.
As it is illustrated in Figure~\ref{fig:scatter_combined_b}, by removing document frequent terms (i.e., $DF(t, Q)>\tau$), we also remove many useful terms (e.g., terms with $RF(t, Q)>0$). In Figure~\ref{fig:scatter_combined_a}, red points are all query terms and in Figure~\ref{fig:scatter_combined_b}, they are query terms with term frequency higher than 5 (i.e., $QTF(t)>5$) --- where we got the best performance. 
Comparing Figures~\ref{fig:scatter_combined_a} and ~\ref{fig:scatter_combined_b} shows that by pruning out terms with $QTF(t)<5$, on the one hand, we are removing considerable amount of noisy terms; on the other hand, we are removing useful terms too. In addition, remained terms are not purely useful terms because they are still contaminated with the noisy~terms (e.g., terms with $RF(t, Q)<0$). %We conclude that we achieve a trivial improvement over the baseline because proposed reduction techniques cannot precisely filter the noisy terms out. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[t!]
%\begin{centering}
...
\begin{centering}
\subfigure[Red points are all terms in the query (e.g, $t \in Query$).\label{fig:scatter_combined_a}]{\includegraphics[width=7cm]{figs/qterm_rf_df.eps}} \hspace*{.1cm} 
\subfigure[Red points are query terms where $\mathit{QTF}$ is higher than 5 (i.e., $t \in Query \; \bigwedge \; QTF(t)>5$).\label{fig:scatter_combined_b}]{\includegraphics[width=7cm]{figs/df-rf-tauline.eps}} 
\par\end{centering} 
...
could not retain only oracular query terms and the reformulated query was contaminated with sufficient 
