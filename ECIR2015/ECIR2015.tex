...
a number that has doubled in the past 15 years.  Hence, helping
...
search constitute only a few words; (ii) patent prior art search is a
...
that represent unfinished patent applications is desirable, since
...
literature~\cite{Magdy2011} have led the research to focus on query
...
sources of expansion terms?  What expansion methods work best?
...
of partial patent application query types and reformulation methods on
standardized patent prior art search datasets from CLEF-IP.
...
Similarly, Rocchio can be use as a QR method.  Basically, the idea is
that once the Rocchio modified query vector have been computed, it is
...
terms with the highest score to built $Q'$. We refer to this approach
as RocchioQR.
...
For query reduction, we can greedily rebuild the query from scratch,
...
approaches spanning synonym lexicons, structure-derived language
...
removal. We removed patent-specific stop-words as described in
...
for CLEF-IP 2010 and hence omit these results due to space
limitations.
...
codes assigned to the query topics for filtering search results to match
...
\begin{itemize}
\item \textbf{Partial patent query type:} We consider that a query of
...
\item \textbf{Query expansion source:} We consider the abstract,
  ...
  expansion terms?  Note that we omit the use of the title as a source
  of expansion terms due to space limitations, simply noting that this
...
\item \textbf{Term selection method:} We consider the different query
  ...
  WSynSet, USynSet. What is the best QE method for patent search?
\end{itemize}
...
\item The best partial application section to use for querying is the
 ...
 \item However, perhaps a better trade-off in terms of effort
   ...
   and querying with the abstract provides a substantial boost in
   retrieval performance compared to the title.  In contrast, querying
   with the claims and description offer only marginal performance gains
   compared to using the abstract.
...
\item When dealing with short queries such as the title or abstract, 
  ...
  diverse term selection may be more helpful for long queries.
...
%  similar inventions.
\item The description section appears not to be a good source for
  expansion, since its content is too broad and it contains  
  many irrelevant terms.
\item In general, generic QE methods like Rocchio tended to outperform
...
\item \textbf{Term selection method:} We consider the different query
  ...
  IPC-StopWords.  What is the best QR method for patent search?
  How do these results compare to QE for the same queries?
\end{itemize}
...
optimal number of terms removed from the original queries (the number
of terms removed that maximizes the performance for each method was
found to be 30). From these results, we make the following
observations:

\begin{enumerate}
\item The best performing QR methods show benefits vs. No QR for all
  queries (i.e., abstract, claims, and description).
\item However, in comparison to the corresponding MAP and PRES results for QE
  from Figure \ref{fig:MAP-CLEF2010} and Figure
  \ref{fig:PRES-CLEF2010}, the best QE and QR methods perform
  roughly the same for abstract and claims queries, but QR outperforms
  QE for description queries.  Hence, the one clear recommendation for QR
  is to use it for long queries such as the description (as one might expect).
\item The term selection methods that provide the best performance
  are, in general, RocchioQR and MMRQR.
\item When dealing with medium-length queries, VSM performs better
  than BM25, while for very long query (i.e. description), BM25 based
  QR methods perform better than VSM based QR
  methods.
\end{enumerate}
...
In this paper we analyzed various query settings for patent prior art
search with partial (incomplete) applications along with general and
specific query reformulation (expansion and reduction) methods on the
patent retrieval corpora of CLEF-IP.

We showed that the description is the best partial application section
to query with, followed by the claims, the abstract, then the title
section.  However, the largest boost in performance comes when
switching from the title to an abstract query; smaller relative boosts
are given by querying with the claims or description.  This is a
critical insight since it is substantially easier for the patent
inventor to draft an abstract rather than a full patent description
and in doing so, still retrieve the majority of prior art that would
have been retrieved with the full description.

We observed that query expansion (QE) methods are useful for short
queries, i.e. title, abstract, and claims, but useless for very long
queries, i.e. the description section.  We also showed that the
description section does not provide the best source of expansion
...
special lexical resources such as synonym lexicons.
...
description section, where the best overall performance results in
this work are for QR when querying with the description.
...
In conclusion, to return to our initial objective to aid the patent
inventor in devcloping an effective pre-application prior art search
strategy, we can conclude from this work that while querying with a
full description in conjunction with generic (non-patent specific)
query reduction methods yields overall best performance, querying with
just an abstract and using a generic QE method yields perhaps the best
