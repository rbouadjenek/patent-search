...
on the query reformulation methods described above. First, we present results
of the Query Expansion experiments. Then, the experiments results for Query Reduction methods.
...
space), best Query Reduction performance results are also obtained when using few
...
of queries extracted from the description. These results tell us that for
almost all methods, the best performance is obtained when removing about 30 terms from
...
In this paper we analyzed general and specific query reformulation methods for
...
to performed patent prior art search. 
In the same vein, we also found that the patent specific fields are more suited as a source for expansion than external sources such as synonym dictionaries. 
...
the longest sections in a patent application. 

Still, the performance of patent prior art is quite low...

For future work we plan to further exploit patent specific meta-data such as
inventor and citation networks.
