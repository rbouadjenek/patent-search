...
The results point out that the term overlap between the queries and the relevant documents
can be very low, see (a), (b), (c). We also observed that when the query is long, for example, 
(d), the overlap between the queries and relevant documents is quite high. 
...
%Moreover, 
QR motivation
In contrast to scientific and technical writers, patent
...
%datasets of CLEF-IP. 
%\end{enumerate}
The rest of the paper is organized as follows: in Section \ref{sec:QueryReformulationForPatents}
...
In the next section, we propose a deep evaluation of query reformulation methods  for patent search, and we attempt to answer the questions we asked
