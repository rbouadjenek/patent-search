...
\begin{centering}
\subfigure[Title query.]{\includegraphics[width=3.5cm]{Results-CIKM2014/jaccard-qTitle-CLEF-IP2010}}\subfigure[Abstract query.]{\includegraphics[width=3.5cm]{Results-CIKM2014/jaccard-qAbstract-CLEF-IP2010}} 
\par\end{centering}

\begin{centering}
\subfigure[Claims query.]{\includegraphics[width=3.5cm]{Results-CIKM2014/jaccard-qClaims-CLEF-IP2010}}\subfigure[Description query.]{\includegraphics[width=3.5cm]{Results-CIKM2014/jaccard-qDescription-CLEF-IP2010}}
...
\emph{Query Expansion} (QE) \cite{Efthimiadis1996} enhance the query with
...
\item [{Query expansion source:}] We consider the title, abstract,
...
QE that we term MMRQE. This method is expected to address a potential
...
QR method, which is expected to rebuild the query from the scratch by
...
is a query reformulation approach that adds terms
...
Following the same motivations than those which led us to explore
diversification for term selection, we propose to greedily rebuild
the query from scratch, while choosing diversified terms (i.e. terms
of the query). We call this approach MMR Query Reduction (MMRQR).
Formally, given a query representation $Q$, we aim to select an optimal
subset of $k$ terms $T_{k}^{*}\subset Q$ (where $|T_{k}^{*}|=k$
and $k<|Q|$) relevant to $Q$ but inherently different from each
...
selection criterion as given in Equation \ref{eq:MMRQE-QR}. Note
that the we used all the sections of the patent documents of the PRF
set to built the document-term matrix of $n$ documents and $m$ terms
shown in Figure~\ref{fig:notation}. Here, we found that $\lambda=0.8$
...
we report MAP, and PRES, which combines Recall with the quality
...
\subsection{Experiments Results for QE}
...
relevant terms that define the invention since a detailed 
summary of the invention is described therein.

According to our experiments, the best source for query expansion is
the claims section. We attribute this to the fact that, 
the claims contained not only relevant, but also, specific terminology, 
since the scope of the invention is described therein.
However, when querying using the claims, other sources of
query expansion provided better performance. 
This may be because claims are very similar between them and contained
specific terms; consequently, the queries lack of diversity
and general terms or synonyms that are used to describe similar inventions. 
It is interesting to notice that the
description is not either a good source for expansion, since its content is too
broad, therefore, it contains many irrelevant terms that hurt the performance.

As expected, we observed that query expansion is not useful for very long
...
We note that using the IPC code definitions (as suggested
...
\subsection{Experiments Results for QR}
...
it contains the core of the invention. This approach has not been implemented
as its authors already point out its poor performance.
...
are helpful. First, we notice that even if there is common
...
and thermal \textit{load}), which likely decrease the quality of
the query. 
...
patent prior art search for partial (incomplete) patent applications on two
...
We also showed that claims is the best section
that works with QE both to query with and to use as a source of query
expansion terms, suggesting that claims should be written at early stages of the
patent application drafting so that they can be use to performed patent prior art search.
We also demonstrate that the novel MMRQE  method improves QE results in
...
that our new QR method MMRQR improve both recall and precision
in many cases. Future work may consist of exploiting
...
\begin{centering}
\subfigure[{\tiny Query Title.}]{\includegraphics[width=8.4cm]{Results-CIKM2014/qTitle-PRES-CLEF-IP2011}}\subfigure[{\tiny Query Abstract.}]{\includegraphics[width=8.4cm]{Results-CIKM2014/qAbstract-PRES-CLEF-IP2011}}
\par\end{centering}

\begin{centering}
\subfigure[{\tiny Query Claims.}]{\includegraphics[width=8.4cm]{Results-CIKM2014/qClaims-PRES-CLEF-IP2011}}\subfigure[{\tiny Query Description.}]{\includegraphics[width=8.6cm]{Results-CIKM2014/qDescription-PRES-CLEF-IP2011}\label{fig:qDescription-PRES-CLEF-IP2011}} 
\par\end{centering}
...
\begin{centering}
\subfigure[{Query Title.}]{\includegraphics[width=4cm]{mmrqrResults/qTitle-MAP-CLEF-IP2010}}\subfigure[{Query Abstract.}]{\includegraphics[width=4cm]{mmrqrResults/qAbstract-MAP-CLEF-IP2010}}\subfigure[{Query Claims.}]{\includegraphics[width=4cm]{mmrqrResults/qClaims-MAP-CLEF-IP2010}}
