...
%General QE method: Rocchio \cite{•} 

% Patent specific QE methods: \cite{Mahdabi2013}, we used the text definitions of the International Patent Classification (IPC) codes assigned to a patent application as a source for query expansion this is denoted as \textbf{IPC Codes}.

% \cite{Magdy2011}: generates candidate synonyms sets (SynSet) for terms, and use it as a source of expansion terms. 

% (i) used the probability associated with the SynSet entries as a weight for each expanded term in the query (\textbf{WSynSet}). Therefore, each term was replaced with its SynSet entries with the probability of each item in the SynSet acting as a weight to the term within the query.  (ii) used uniform weighting for all synonyms of a given term (\textbf{USynSet}).
...
%\caption{Results obtained while using the claims for querying and the abstract as source of query expansion on the CLEF-IP 2010 dataset.}
...
\begin{frame}
\frametitle{MAP and PRES for QE methods on CLEF-IP 2010}
...
\begin{frame}
\frametitle{MAP and PRES for QE methods on CLEF 2011}
%Patent Retrieval Evaluation Score (
...
%Patent Retrieval Evaluation Score (PRES) for QR methods on CLEF 2010
...
%Patent Retrieval Evaluation Score (PRES) for QR methods on CLEF 2011
