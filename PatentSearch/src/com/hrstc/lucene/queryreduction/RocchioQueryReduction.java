...
/**
 * Implements Rocchio's pseudo feedback RocchioQueryReduction algorithm
 * <p>
 ...
 */
public final class RocchioQueryReduction {
    // CONSTANTS
     ...
     */
    public RocchioQueryReduction(TopDocs hits, IndexReader ir, Map<String, Float> parameters, int source, int Nbr_Docs, int Nbr_Terms) throws IOException {
        this.ir = ir;
    ...
    public Query reduceQuery(Query query, String currentField) throws IOException, ParseException {
        ...
        float alpha = parameters.get(RocchioQueryReduction.ROCCHIO_ALPHA_FLD);
        float beta = parameters.get(RocchioQueryReduction.ROCCHIO_BETA_FLD);
        float gamma = parameters.get(RocchioQueryReduction.ROCCHIO_GAMMA_FLD);
        float decay = parameters.get(RocchioQueryReduction.DECAY_FLD);

        // Adjust termTitle features of the docs with alpha * query; and beta; and assign weights/boost to terms (tf*idf)
        Query reducedQuery = adjust(query, currentField, alpha, beta, gamma, decay);
     ...
     *
     * @return reducedQuery with boost factors adjusted using Rocchio's
     * algorithm
     *
     * @throws IOException
     * @throws ParseException
     */
    public Query adjust(Query query, String currentField, float alpha, float beta, float gamma, float decay)
...
//        Map<String, TermQuery> irrrelevantDocsTerms = new HashMap<>();
        ...
        int termCount = expandedQueryTerms.size();
...
//        }
        ...
        int querySize = queryTerms.size() - Nbr_Terms;
        if (querySize < 0) {
            querySize = 0;
        }
        expandedQuery = mergeQueries(expandedQueryTerms, querySize);
