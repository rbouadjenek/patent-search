...
In this chapter, first, we briefly explain the structure of patents, then we cover both generic IR methods and patent-specific IR methods.
\section{Structure of Patents}
\label{StructureofPatents}
A patent is a structured document, which consists of four main sections:
title, abstract, description, and claims. It also contains tables, mathematical and chemical formulas, citations,
technical drawing, and meta-data.
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
In this section, we briefly explain the main sections and some meta-data of a patent document that are commonly used in a patent retrieval system as follows:\\\\
\textbf{(1) Title}
\ \\ 
...
\textbf{(2) Abstract}
\ \\ 
The abstract of the patent document is a short paragraph that contains
...
are not novel. This usually happens because patent applicants try to generalise their invention as
...
the claims are submitted in the language of the document. 
Claims translations are only provided for a granted patent.\\\\
Nonetheless, patents contain
tables, mathematical and chemical formulas, citations,
technical drawing, meta-data (e.g., applicant, inventor, IPC codes, and publication date) and other 
additional material that can be used to improve the retrieval effectiveness.
As it will be described in Section~\ref{sec:metadata}, IPC codes and citations has been widely applied in patent retrieval. Hence, we describe them as follows: \\\\
\textbf{(5) International Patent Classification Code}
\ \\ 
In 1971, the Strasbourg Agreement established the International Patent Classification (IPC) 
under the World Intellectual Property Organization (WIPO) that divides technology into eight discrete sections~\citep{harris2010comparison}. The goal of this
agreement was to overcome the difficulties caused by using diverse national patent classification~systems.
...
classes. Class C07, for example, deals with ``Organic Chemistry''. Classes are further subdivided into more than 600 
subclasses. Subclass C07C, for instance, deals with ``Acyclic or Carbocyclic Compounds''. Subclasses are then further 
divided into main groups and subgroups. Main group symbols end with ``/00''. Ten percent of all IPC groups are main
groups. For example, the main group C07C 35/00 deals with ``Compounds having at
...
indicating the subject to which the invention relates; this is the Main Code. They may also be assigned further classification and 
...
is often inaccurate and can result in an answer set that does not satisfy the user's information need~\citep[pp. 3--10]{manning2008introduction}. 
...
\begin{figure}[htpb]
   \centering
   \includegraphics[width=.70\textwidth,height=55mm]{figs/generalIR.png}
   \caption{Illustration of the process in a generic $\mathit{IR}$ system.}  
   \label{fig:generalir} 
\end{figure}
\FloatBarrier 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Summary}
This section covered all background material required to understand 
both generic and patent-specific IR. It also included literature reviews over previous 
