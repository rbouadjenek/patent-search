...
that represent unfinished patent applications would be desirable, since
...
\item The best partial application section to use for querying is the
 ...
 \item However, perhaps a better trade-off in terms of effort
   ...
   retrieval performance compared to the title (almost doubling MAP performance).  
...
\item However, in comparison to the corresponding MAP and PRES results
  for QE from Figure \ref{fig:MAP-CLEF2010} and Figure
  \ref{fig:PRES-CLEF2010}, the best QE and QR methods perform roughly
  the same for abstract and claims queries, but the best QR slightly
  outperforms the best QE for description queries.  Hence, the best
  overall retrieval result in this work in terms of both MAP and PRES
  comes from using QR with a description query.
...
CLEF-IP patent retrieval corpus.
...
title section.  However, the largest boost in performance comes when
...
and in doing so, still manage to retrieve the majority of prior art
that would have been retrieved with the full description.
