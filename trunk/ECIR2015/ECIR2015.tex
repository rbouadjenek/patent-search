...
and patent-specific query reformulation methods; in Section
\ref{sec:Evaluation}, we present the evaluation results and analysis
to answer the above questions; and in Section \ref{sec:Conclusion}, we
conclude with key observations from our results along with possible
...
\subfour{Synonyms Sets for Patent Query Expansion:}
...
\item When dealing with more medium-length queries such as abstract or claims,
...
\begin{enumerate}
\item Query reduction is often useful for medium-length queries (i.e. abstract
...
\item When dealing with medium-length queries, VSM performs better than BM25, while
...
are considered as medium-length sections in a patent application. The improvement
