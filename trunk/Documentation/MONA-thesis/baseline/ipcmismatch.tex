...
\begin{figure}[t!]
\begin{centering}
\subfigure[non-relevant retrieved patents (False Negative (FN) patents)\label{fig:ipcoverlap_a}]{\includegraphics[width=6.5cm]{figs/ipcOverlap-FNs.png}} 
\hspace*{0.5cm}  \subfigure[relevant retrieved patents (True Positive (TP) patents)\label{fig:ipcoverlap_b}]{\includegraphics[width=6.5cm]{figs/ipcOverlap-TPs.png}} 
...
\begin{figure}[t!]
\begin{centering}
\subfigure[Filter: The first two components (Section and Class)\label{fig:ipc1stTwoElements_a}]{\includegraphics[width=6.5cm, height=5cm]{figs/ipc1stTwoElements.png}} 
\\[1ex]%
\subfigure[The distribution of the number of patents should be ranked for each query over all test queries (1303).
In average, the matching process for each query is done over $ 99754 $ documents instead of the whole collection (2.6 million documents) which reduce the computational time dramatically.\label{fig:ipc1stTwoElements_b}]{\includegraphics[width=9.5cm , height=6cm]{figs/firstTwoIpcFilter-histo.png}} 

\par\end{centering} 
\protect\caption{Applying first two IPC code components for filtering}
\label{fig:ipc1stTwoElements}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure}[htpb]
...
\begin{figure}[t!]
\begin{centering}
\subfigure[Filter: The first two components (Section and Class)\label{fig:ipc1stElements_a}]{\includegraphics[width=6.5cm, height=5cm]{figs/ipc1stElement.png}} 
\\[1ex]%
\subfigure[The distribution of the number of patents should be ranked for a query over all test queries (1303), after applying the IPC filter: ``\textit{only the first component of the IPC code}".
In average, the matching process for each query is done over $ 415828 $ documents instead of the whole collection (2.6 million documents). This number is much higher than using more restricted filters and will not be efficient computationally.\label{fig:ipc1stElements_b}]{\includegraphics[width=9.5cm , height=6cm]{figs/firstIpcFilter-histo.png}} 

\par\end{centering} 
\protect\caption{Applying the first IPC code components for filtering}
