...
Terms, which have the $\mathit{PRF}$ score higher than the threshold $\tau$ ($PRF(t)>\tau$), are selected from the Patent Query to reformulate a reduced query. Figure~\ref{fig:queryreduc} shows that this approach is also unsuccessful at achieving a notable improvement over the baseline.
